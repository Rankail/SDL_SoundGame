...
TTF_Font* FontLib::GetFont(const std::string& name, int32_t pointSize)
{
    ...
    std::cout << "Could not find Font '" << name << "':" << pointSize << std::endl;
    return nullptr;
}
...
bool FontLib::RemoveFont(const std::string& name, int32_t pointSize)
{
    ...
    if (it == m_Fonts.end())
    {
        std::cout << "Tried to remove non-existing Font '" << name << "'" << std::endl;
        return false;
    }
    ...
    if (it2 == it->second.end())
    {
        std::cout << "Tried to remove non-existing Font '" << name << "':" << pointSize << std::endl;
        return false;
    }
    it->second.erase(it2);
    return true;
